...
export async function finalizeContest(contestId, forceDraw = false, winnerTeam = null) {
    logger.debug(`finalizeContest: 正在结算比赛: 比赛 ID ${contestId}, forceDraw: ${forceDraw}, winnerTeam: ${winnerTeam}`);
    ...
    try {
        ...
        if (rows.length === 0) {
            await conn.rollback();
            return { success: false, message: '未找到该比赛' };
        }
        ...
        if (contest.status === 2) {
            await conn.commit();
            return { success: true, message: '比赛已经结算' };
        }
        ...
        const [
            ...
            conn.execute('SELECT * FROM contest_teams WHERE contest_id = ?', [contest.id]),
            conn.execute('SELECT * FROM contest_participants WHERE contest_id = ?', [contest.id]),
            conn.execute('SELECT * FROM contest_problems WHERE contest_id = ?', [contest.id])
        ]);
        ...
        if (contest.rated && allUsernames.length > 0) {
            ratingChanges = {};
            // 获取选手的当前 Rating 和参赛次数 (并行获取)
            const [
                [users],
                [histories]
            ] = await Promise.all([
                conn.execute('SELECT username, rating FROM user WHERE username IN (?)', [allUsernames]),
                conn.execute('SELECT username, COUNT(*) as count FROM contest_participants WHERE username IN (?) AND contest_id != ? GROUP BY username', [allUsernames, contest.id])
            ]);
        ...
        logger.error(`finalizeContest 错误: ${err.message}`);
 ...
 */
async function contest_final(ctx) {
    const { contestId } = ctx.request.body;
    if (!contestId) {
        ctx.status = 400;
        ctx.body = { success: false, message: '比赛 ID 不能为空' };
        return;
    }
    ...
    try {
        ...
        ctx.status = result.success ? 200 : (result.message === '未找到该比赛' ? 404 : 500);
        ctx.body = result;
    } catch (err) {
        logger.error(`contest_final 错误: ${err.message}`);
        ctx.status = 500;
