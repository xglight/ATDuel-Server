...
async function roomReady(ctx) {
    ...
    try {
        ...
        // 获取房间数据并加锁
        const [roomRows] = await conn.execute('SELECT id, setting_mode, setting_rating_lowest, setting_rating_highest, setting_problem_count, rated FROM room WHERE url = ? FOR UPDATE', [roomUrl]);
